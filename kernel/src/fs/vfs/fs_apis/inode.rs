...
pub trait Inode: Any + InodeIo + Send + Sync {
    ...
    /// Creates an unnamed regular file in this directory.
    ///
    /// The returned inode must start with zero hard links and must not appear in
    /// directory iteration. If `hard_linkability` is `HardLinkability::Linkable`,
    /// the inode may be linked explicitly later; otherwise, it must remain
    /// unlinkable.
    fn create_tmpfile(
        &self,
        mode: InodeMode,
